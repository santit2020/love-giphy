...
if not BOT_TOKEN:
    print("❌ TELEGRAM_BOT_TOKEN is missing! Please set it in Render → Environment Variables.")
    exit(1)

if not GIPHY_API_KEY:
    print("❌ GIPHY_API_KEY is missing! Please set it in Render → Environment Variables.")
    exit(1)
...
# =========================
# GIF SENDER  (FIX: now uses async httpx instead of blocking requests)
# =========================
...
def schedule_user_job(application, chat_id, hours):
    ...
    print(f"⏰ scheduled job for {chat_id}")
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_send_message(
        context.bot,
        chat_id,
        "💔 stopped love messages. Say /start anytime to come back 💖"
    )
...
async def main():
    ...
    for chat_id, data in users.items():
        if data.get("subscribed"):
            ...
            schedule_user_job(application, chat_id, hours)

    print("💖 bot is alive! Polling for messages...")
